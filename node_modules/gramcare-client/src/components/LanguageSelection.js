...
const LanguageSelection = ({ onLanguageSelect, supportedLanguages, setCurrentLanguage }) => {
  ...
  // Use supportedLanguages prop if available, otherwise use defaultLanguages
  const languagesToRender = supportedLanguages && Object.keys(supportedLanguages).length > 0
    ? Object.entries(supportedLanguages).map(([code, name]) => ({
        code,
        name,
        flag: defaultLanguages.find(lang => lang.code === code)?.flag || '🌐', // Fallback flag
        nativeName: defaultLanguages.find(lang => lang.code === code)?.nativeName || name // Fallback native name
      }))
    : defaultLanguages;
