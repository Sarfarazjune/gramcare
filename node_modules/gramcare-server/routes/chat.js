...
const GREETING_MESSAGES = {
  ...
  'te': 'నమస్కారం! నేను గ్రామకేయర్, మీ ఆరోగ్య సహాయకుడు। ఈరోజు నేను మీకు ఎలా సహాయం చేయగలను?'
};
...
const FALLBACK_MESSAGES = {
  ...
  'bn': 'দুঃখিত, আমি এটা বুঝতে পারিনি। আপনি কি দয়া করে আপনার প্রश্নটি আবার বলতে পারেন?',
...
router.post('/', async (req, res) => {
  try {
    const { message, language = 'english', userId = 'anonymous', sessionId, whatsappNumber } = req.body;
    ...
    if (whatsappNumber) {
      try {
        ...
        await sendWhatsAppMessage(whatsappNumber, `GramCare received your message: "${userMessage}". We are processing it and will respond shortly.`);
        response = `Message sent to WhatsApp number ${whatsappNumber}.`;
        responseType = 'whatsapp_sent';
        confidence = 1.0;
      } catch (twilioError) {
        console.error('Twilio WhatsApp send error:', twilioError);
        response = 'Failed to send message via WhatsApp. Please check the number and try again.';
        responseType = 'whatsapp_error';
        confidence = 0.0;
      }
    } else {
    ...
    if (whatsappNumber && responseType !== 'whatsapp_sent' && responseType !== 'whatsapp_error') {
      try {
        await sendWhatsAppMessage(whatsappNumber, response); // Send the AI-generated response
        console.log(`AI response sent to WhatsApp number ${whatsappNumber}.`);
      } catch (twilioError) {
        console.error('Twilio WhatsApp AI response send error:', twilioError);
      }
    }
